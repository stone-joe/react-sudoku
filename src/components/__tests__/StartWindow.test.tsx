...
describe('StartWindow', () => {
  it('should present the user with each option', () => {
    ...
    // test
    const { getByText, getByRole } = render(<StartWindow levels={['easy', 'medium', 'hard']} show={true} onSelect={selectHandler} />);
    fireEvent.click(getByText('Easy'));
    fireEvent.click(getByText('Medium'));
    fireEvent.click(getByText('Hard'));
    // verify
    expect(getByRole('alert')).toHaveStyle('display: block');
  ...
  it('should hide the window', () => {
    // test
    const { getByRole } = render(<StartWindow levels={['easy', 'medium', 'hard']} show={false} onSelect={jest.fn()} />);
    // verify
    expect(getByRole('alert', { hidden: true })).toHaveStyle('display: none');
  });
});